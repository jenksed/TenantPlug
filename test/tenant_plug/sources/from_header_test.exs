defmodule TenantPlug.Sources.FromHeaderTest do
  ...
  describe "extract/2" do
    ...
    test "returns error when header is missing" do
      conn = conn(:get, "/")
      assert FromHeader.extract(conn, %{}) == {:error, :not_found}
    end
    ...
    test "returns error when header is only whitespace" do
      ...
      assert FromHeader.extract(conn, %{}) == {:error, :empty_header}
    end
  ...
  describe "mapper option" do
    ...
    test "mapper can return complex data structures" do
      ...
      assert FromHeader.extract(conn, opts) ==
               {:ok, expected_tenant, %{source: :header, raw: "acme"}}
    end
  ...
  describe "edge cases" do
    test "handles very long header values" do
      ...
      assert FromHeader.extract(conn, %{}) ==
               {:ok, long_value, %{source: :header, raw: long_value}}
    end
    ...
    test "handles special characters in header values" do
      ...
      assert FromHeader.extract(conn, %{}) ==
               {:ok, special_value, %{source: :header, raw: special_value}}
    end
    ...
    test "handles unicode characters in header values" do
      ...
      assert FromHeader.extract(conn, %{}) ==
               {:ok, unicode_value, %{source: :header, raw: unicode_value}}
    end
  end
end
