defmodule TenantPlug.Sources.FromSubdomainTest do
  ...
  describe "extract/2 with default configuration" do
    test "extracts subdomain from valid host" do
      ...
      assert FromSubdomain.extract(conn, %{}) ==
               {:ok, "acme", %{source: :subdomain, raw: "acme.example.com"}}
    end
    ...
    test "returns error for empty subdomain" do
      ...
      assert FromSubdomain.extract(conn, %{}) == {:error, :empty_subdomain}
    end
  end
  ...
  describe "extract/2 with host_split_index option" do
    test "extracts subdomain from specified index" do
      ...
      assert FromSubdomain.extract(conn, opts) ==
               {:ok, "acme", %{source: :subdomain, raw: "api.acme.example.com"}}
    end
    ...
    test "returns error when index is out of bounds" do
      ...
      assert FromSubdomain.extract(conn, opts) == {:error, :no_subdomain}
    end
    ...
    test "handles negative index gracefully" do
      ...
      opts = %{host_split_index: -1}
      assert FromSubdomain.extract(conn, opts) == {:error, :invalid_index}
    end
  end
  ...
  describe "extract/2 with exclude_subdomains option" do
    test "excludes specified subdomains" do
      ...
      assert FromSubdomain.extract(conn, opts) == {:error, :excluded_subdomain}
    end
    ...
    test "handles multiple excluded subdomains" do
      test_cases = [
        {"www.example.com", {:error, :excluded_subdomain}},
        {"api.example.com", {:error, :excluded_subdomain}},
        {"admin.example.com", {:error, :excluded_subdomain}},
    ...
    test "empty exclude list allows all subdomains" do
      ...
      assert FromSubdomain.extract(conn, opts) ==
               {:ok, "www", %{source: :subdomain, raw: "www.example.com"}}
    end
  end
  ...
  describe "edge cases" do
    test "handles host with port" do
      ...
      assert FromSubdomain.extract(conn, %{}) ==
               {:ok, "acme", %{source: :subdomain, raw: "acme.example.com:4000"}}
    end

    test "handles IPv4 addresses gracefully" do
      conn = %Plug.Conn{conn(:get, "/") | host: "192.168.1.1"}

      assert FromSubdomain.extract(conn, %{}) == {:error, :ip_address}
    end
    ...
    test "handles very long subdomains" do
      ...
      assert FromSubdomain.extract(conn, %{}) ==
               {:ok, long_subdomain, %{source: :subdomain, raw: host}}
    end
    ...
    test "handles international domain names" do
      ...
      assert FromSubdomain.extract(conn, %{}) ==
               {:ok, "tenant", %{source: :subdomain, raw: "tenant.例え.テスト"}}
    end
